...
from customer_service.hjf_ai.core.slots import Slot, create_slot
from customer_service.hjf_ai.dialogue_understanding.stack.dialogue_stack import DialogueStack
from customer_service.hjf_ai.shared.constants import ACTION_LISTEN, DEFAULT_SENDER_ID
...
class DialogueStateTracker:
    ...
    状态管理：
        使用统一的 dialogue_stack管理所有对话上下文（Flow、搜索、闲聊等）
        active_flow 是从 dialogue_stack派生的计算属性。
    ...
    def __init__(
        ...
        # 采用Dict是为了方便根据slot_name来过滤数据，不用通过Slot.slot_name
    ...
    def active_flow(self) -> Optional[str]:
        ...
        return frame.frame_id if frame else None
    ...
    def update_with_message(self, message: UserMessage) -> None:
        ...
        """
        # 保存之前的轮次
        # 如果 _current_turn 不为空，里面存储的是上一轮的数据， 此时需要保存
        if self._current_turn is not None:
            self._save_current_turn()

        # 开始新轮次
        # _current_turn 为空时，用当前消息覆盖【当前塞入用户消息】
    ...
    def set_slot(
        ...
        if slot_name in self.slots:
            ...
            # 创建新槽位时 initial_value应该是None，这样reset()才能正确重置
            self.slots[slot_name] = create_slot(name=slot_name, inital_value=None)
            self.slots[slot_name].value = value
        self.updated_at = time.time()
    ...
    def set_latest_action(self, action_name: str) -> None:
        ...
        if self._current_turn:
            self._current_turn.action = action_name
        self.updated_at = time.time()
    ...
    def end_flow(self) -> Optional[str]:
        ...
        flowId = flow_frame.flow_id

        # 弹出到该Flow（包括它上面的所有帧）
        self.dialogue_stack.pop_to_flow(flowId)

        # 再弹出Flow本身
        self.dialogue_stack.pop()

        # 更新flow_history中对应项的结果信息
        for history in self.flow_history:
            if history["flow_name"] == flowId and history["ended_at"] is None:
                history["ended_at"] = time.time()
    ...
    def record_pattern(self, pattern_type: str, completed: bool = True) -> None:
        ...
        将内置Pattern（如L：chitchat、search、cannot_handle等）的执行记录
        添加到flow_history中，以便在Inspect页面统一展示

        参数
            pattern_type: Pattern 类型，如 "chitchat"、"search"、"cannot_handle"、
                         "completed"、"human_handoff"
            completed：是否执行完成，默认为True
        """
    ...
    def from_dict(
        ...
        tracker = cls(
            send_id=data.get("send_id", DEFAULT_SENDER_ID),
            slots=slots,
        )

        # 恢复dialogue_stack
        if "dialogue_stack" in data:
            tracker.dialogue_turns = DialogueStack.from_dict(data["dialogue_stack"])
        ...
        for turn_data in data.get("dialogue_turns", []):
            ...
            if turn_data.get("user_message"):
                turn.user_message = UserMessage.from_dict(turn_data.get("user_message"))
    ...
    def __repr__(self) -> str:
        return (
            f"DialogueStateTracker(sender_id={self.send_id},"
