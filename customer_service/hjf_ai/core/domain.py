...
定义对话系统的领域(Domain), 包括槽位、动作、响应模版等。
Domain是对话系统配置等核心，描述了系统能够处理等对话范围。
"""
...
from customer_service.hjf_ai.shared.constants import ACTION_LISTEN, ACTION_RESTART, ACTION_SESSION_START, \
    ACTION_DEFAULT_FALLBACK, DEFAULT_DOMAIN_PATH
from customer_service.hjf_ai.shared.exceptions import DomainValidationError
from customer_service.hjf_ai.shared.yaml_loader import merge_yaml_files, read_yaml_file
...
class Domain:
    ...
    def get_response(self, response_name: str) -> List[ResponseTemplate]:
        ...
        参数：
            response_name：响应名称（如：utter_greet）
