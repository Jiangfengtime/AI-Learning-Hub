...
class StackFrame(ABC):
    ...
    state：帧的当前状态
    """
    frame_id: str = field(default=generate_frame_id())
...
class FlowStackFrame(StackFrame):
    ...
    Attributes：
        flow_id: Flow的id
        step_id: 当前步骤id
        flow_frame_type: Flow栈类型(regular, interrupt, link)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "FlowStackFrame":
        ...
        # 支持flow_frame_type和旧的frame_type字段
...
class CompletedStackFrame(StackFrame):
    ...
    由FlowPolicy在Flow完成后自动压入
    由EnterpriseSearchPolicy处理，生成询问用户是否还有其他需要的响应。
...
class HumanHandoffStackFrame(StackFrame):
    ...
    标识需要将对话转接给人工客服
    由ActionHumanHandoff压入，由EnterpriseSearchPolicy处理
